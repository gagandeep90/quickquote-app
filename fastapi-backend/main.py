...
# Allow your React frontend to talk to us
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def explode_all_blocks(msp):
    """Turn all INSERT blocks into raw geometry."""
    ...
    try:
        ...
        for e in msp:
            ...
            # ARC
                ...
                # sample the arc in small segments
                steps = 30
            ...
            # SPLINE (robust fallback)
                ...
                if hasattr(e, "fit_points"):
                    ...
                    if isinstance(fp, (list,tuple)): raw_points = fp
                # 2) else try control_points
                if not raw_points and hasattr(e, "control_points"):
                    cp = e.control_points
                    if callable(cp): cp = cp()
                    if isinstance(cp, (list,tuple)): raw_points = cp
                # 3) if still flat list of numbers, group them by 3 (x,y,z)
                if raw_points and isinstance(raw_points[0], (int,float)):
                    coords = raw_points
                    raw_points = [(coords[i], coords[i+1]) for i in range(0, len(coords), 3)]
